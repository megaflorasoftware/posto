...
assert(dataEntryValues(json, jsonEntry)?.profile && typeof dataEntryValues(json, jsonEntry)?.profile === "object", "missing object paths materialize");
...
assert(dataDocumentEntries(json).map((entry) => entry.id).join(",") === "two", "JSON entry removed");

const yaml = parseDataDocument(`# authors
- id: one
  title: One # keep me
`, "yaml");
...
assert(dataDocumentEntries(parseDataDocument(yamlSource, "yaml")).length === 2, "YAML array round trip");

const tomlRecord = parseDataDocument(`[one]
title = "One"

[two]
title = "Two"
`, "toml");
assert(dataDocumentEntries(tomlRecord).map((entry) => entry.id).join(",") === "one,two", "TOML keyed entries");
const tomlOne = dataDocumentEntries(tomlRecord)[0];
setDataValue(tomlRecord, [...tomlOne.path, "title"], "Updated");
assert(dataEntryValues(parseDataDocument(serializeDataDocument(tomlRecord), "toml"), tomlOne)?.title === "Updated", "TOML record round trip");

const tomlArray = parseDataDocument(`[[authors]]
id = "one"
title = "One"
...
title = "Two"
`, "toml");
assert(dataDocumentEntries(tomlArray).map((entry) => entry.id).join(",") === "one,two", "TOML array-of-tables entries");
...
console.log("data-document tests passed");
